...
const compareLocalVersionAgainstNpmVersion = ( { version: localVersion, name } ) => new Promise( (resolve, reject ) => {
    const buffer = [];
...
prepareWorkspace()
    ...
    .then(
        ...
        ( err ) => { console.log( err ); throw new Error( { reason: `Failed to execute one or more tasks for “${name}”!` } ) }
    );
